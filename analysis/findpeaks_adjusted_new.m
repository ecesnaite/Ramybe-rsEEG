...
if nargout > 0
  ...
  if needWidth
    [Ypk,Xpk,Wpk,Ppk,wxPk] = assignFullOutputs(y,x,iPk,wxPk,bPk,yIsRow,xIsRow);
  else
...
for i=1:n
  ...
  refHeight = (y(iPk(i))+base(i))/10; %ELENA: changed from 2 to 8
...
xc = xa + (xb-xa) .* (0.1*(yc+bc)-ya) ./ (yb-ya);% ELENA: changed from 0.5 to 0.125
...
%--------------------------------------------------------------------------
function [YpkOut,XpkOut,WpkOut,PpkOut,wxPk] = assignFullOutputs(y,x,iPk,wxPk,bPk,yIsRow,xIsRow)
coder.internal.prefer_const(yIsRow,xIsRow);
...
if strcmp(refW,'halfheight')
  hm = 0.1*y(iPk); %ELENA for 1/8 of the peak
else
  hm = 0.1*(y(iPk)+bPk);
end
